# Série Mesto (2/5) — Doprava

@ img/mesto_02_tram_stop.jpg | Tram devant la gare de Košice — Wikimedia Commons, CC BY-SA

Les transports en commun. Et la distinction entre **ísť** (un trajet précis) et **chodiť** (une habitude), déjà annoncée dans la série Čas.

---

## Les nouveaux mots

| Slovaque | Français |
|----------|----------|
| autobus | le bus |
| električka | le tram |
| vlak | le train |
| zastávka | l'arrêt |
| cestovný lístok | le ticket |
| chodiť | aller habituellement |
| MHD | les transports en commun |

---

## La grammaire du jour

### Ísť ou chodiť ?

| Slovaque | Français |
|----------|----------|
| Idem do práce. | Je vais au travail (maintenant). |
| Chodím do práce autobusom. | Je vais au travail en bus (d'habitude). |
| Ideš na zastávku? | Tu vas à l'arrêt (là, maintenant) ? |
| Chodíš pešo? | Tu y vas à pied (en général) ? |

*Ísť* décrit un déplacement précis et ponctuel. *Chodiť* décrit une habitude répétée. Le choix entre les deux ne dépend pas du moyen de transport, mais de la régularité de l'action.

### Chodiť au présent

| Slovaque | Français |
|----------|----------|
| ja chodím | je vais (habituellement) |
| ty chodíš | tu vas |
| on / ona chodí | il / elle va |
| my chodíme | nous allons |

---

## Des phrases

! Chodím do práce autobusom.
> Je vais au travail en bus.
> Chodím = je vais (d'habitude)
> do práce = au travail
> autobusom = en bus

! Dnes idem pešo.
> Aujourd'hui j'y vais à pied.
> Dnes = aujourd'hui
> idem = j'y vais
> pešo = à pied

! Kde je zastávka?
> Où est l'arrêt ?
> Kde = où
> je = est
> zastávka = l'arrêt

! Chcem cestovný lístok.
> Je veux un ticket.
> Chcem = je veux
> cestovný lístok = un ticket

! Električka ide o piatej.
> Le tram part à cinq heures.
> Električka = le tram
> ide = part
> o piatej = à cinq heures

! Chodíš vždy vlakom?
> Tu prends toujours le train ?
> Chodíš = vas-tu (d'habitude)
> vždy = toujours
> vlakom = en train

! Autobus mešká desať minút.
> Le bus a dix minutes de retard.
> Autobus = le bus
> mešká = est en retard
> desať minút = dix minutes

! MHD je v Košiciach dobrá.
> Les transports en commun sont bons à Košice.
> MHD = les transports en commun
> je = est
> v Košiciach = à Košice
> dobrá = bonne

---

## On révise

! Chodím do parku každý deň.
> Je vais au parc tous les jours.
> Chodím = je vais (d'habitude)
> do parku = au parc
> každý deň = tous les jours

! Idem na námestie. Ideš so mnou?
> Je vais sur la place. Tu viens avec moi ?
> Idem = je vais
> na námestie = sur la place
> Ideš = tu viens
> so mnou = avec moi

! Múzeum je zatvorené. Chodíme tam v sobotu.
> Le musée est fermé. On y va d'habitude le samedi.
> Múzeum = le musée
> je = est
> zatvorené = fermé
> Chodíme = nous allons (d'habitude)
> tam = là
> v sobotu = le samedi

---

## 🇸🇰 Coin slovaque

**MHD, l'abréviation que tout le monde utilise.** Pour *mestská hromadná doprava*, transport urbain collectif, ce sigle apparaît sur les applications, les tickets et les panneaux. Personne ne prononce la version longue au quotidien.

**Le tram de Košice roule depuis 1891, à l'origine tiré par des chevaux.** Il a été électrifié en 1913, et reste depuis un pilier de la mobilité en ville, avec des rames modernes à côté de vieux modèles encore en service sur certaines lignes. Les tickets s'achètent en borne ou par application.

**La gratuité pour les jeunes et les seniors.** Depuis quelques années, les moins de 26 ans et les plus de 62 ans voyagent gratuitement sur les trains nationaux. Une bonne nouvelle à connaître si tu voyages avec de la famille slovaque.

---

## Vocabulaire complémentaire

| Slovaque | Français |
|----------|----------|
| pešo | à pied |
| bicykel | le vélo |
| auto | la voiture |
| taxík | le taxi |
| lístok | le billet |
| stanica | la gare, la station |
| smer | la direction |

---

## Encore quelques phrases

! Chodím do školy pešo.
> Je vais à l'école à pied.
> Chodím = je vais (d'habitude)
> do školy = à l'école
> pešo = à pied

! Mám bicykel, ale dnes idem autom.
> J'ai un vélo, mais aujourd'hui j'y vais en voiture.
> Mám = j'ai
> bicykel = un vélo
> ale = mais
> dnes = aujourd'hui
> idem = j'y vais
> autom = en voiture

! Zavolám taxík.
> Je vais appeler un taxi.
> Zavolám = je vais appeler
> taxík = un taxi

! Aký je smer?
> Quelle est la direction ?
> Aký = quelle
> je = est
> smer = la direction

! Stanica je ďaleko, choď električkou.
> La gare est loin, prends le tram.
> Stanica = la gare
> je = est
> ďaleko = loin
> choď = prends, va
> električkou = en tram
