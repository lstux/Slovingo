# Série Mesto (3/5) — Kade mám ísť?

@ img/mesto_03_square.jpg | La rue Hlavná à Košice — Wikimedia Commons, CC BY-SA

Demander son chemin et le suivre. C'est ici qu'apparaît l'**impératif**, la forme qu'on utilise pour donner une instruction.

---

## Les nouveaux mots

| Slovaque | Français |
|----------|----------|
| choď | va |
| choďte | allez (vouvoiement) |
| rovno | tout droit |
| vľavo | à gauche |
| vpravo | à droite |
| za rohom | après le coin |
| priamo | juste, directement |

---

## La grammaire du jour

### L'impératif de base

| Slovaque | Français |
|----------|----------|
| Choď! | Va ! (tutoiement) |
| Choďte! | Allez ! (vouvoiement) |
| Zabočte vľavo! | Tournez à gauche ! |
| Choďte rovno! | Allez tout droit ! |

L'impératif se retient d'abord tel quel, sans en construire la règle : ce sont des formes très fréquentes qu'on entend partout dans la rue.

### Situer un lieu

| Slovaque | Français |
|----------|----------|
| Je to za rohom. | C'est après le coin. |
| Je to vedľa banky. | C'est à côté de la banque. |
| Je to pred kostolom. | C'est devant l'église. |
| Je to blízko. | C'est près d'ici. |

---

## Des phrases

! Prepáčte, kde je pošta?
> Excusez-moi, où est la poste ?
> Prepáčte = excusez-moi
> kde = où
> je = est
> pošta = la poste
+ « prepáčte » sert à aborder un inconnu poliment.

! Choďte rovno a potom vľavo.
> Allez tout droit puis à gauche.
> Choďte = allez
> rovno = tout droit
> a potom = et ensuite
> vľavo = à gauche

! Je to za rohom.
> C'est après le coin.
> Je to = c'est
> za rohom = après le coin

! Choď vpravo, nie vľavo!
> Va à droite, pas à gauche !
> Choď = va
> vpravo = à droite
> nie = pas
> vľavo = à gauche

! Múzeum je priamo pred vami.
> Le musée est juste devant vous.
> Múzeum = le musée
> je = est
> priamo = juste
> pred vami = devant vous

! Zastávka je blízko, choďte rovno.
> L'arrêt est proche, allez tout droit.
> Zastávka = l'arrêt
> je = est
> blízko = proche
> choďte rovno = allez tout droit

! Kade mám ísť?
> Par où dois-je aller ?
> Kade = par où
> mám ísť = dois-je aller

! Ďakujem za pomoc!
> Merci pour l'aide !
> Ďakujem = merci
> za pomoc = pour l'aide

---

## On révise

! Prepáčte, kde je zastávka autobusu?
> Excusez-moi, où est l'arrêt de bus ?
> Prepáčte = excusez-moi
> kde = où
> je = est
> zastávka autobusu = l'arrêt de bus

! Choďte rovno, kostol je na námestí.
> Allez tout droit, l'église est sur la place.
> Choďte rovno = allez tout droit
> kostol = l'église
> je = est
> na námestí = sur la place

! Banka je vedľa múzea, za rohom.
> La banque est à côté du musée, après le coin.
> Banka = la banque
> je = est
> vedľa múzea = à côté du musée
> za rohom = après le coin

---

## 🇸🇰 Coin slovaque

**Prepáčte ouvre toutes les portes.** Ce mot d'excuse poli sert à interpeller un inconnu, à se faufiler dans une foule ou à s'excuser d'un vrai impair. C'est probablement l'un des mots les plus utiles du répertoire touristique.

**Les Slovaques indiquent volontiers le chemin à pied.** Demander sa route dans la rue reste courant et bien accueilli, contrairement à d'autres capitales où l'on préfère sortir son téléphone. Une explication orale accompagnée d'un geste du bras est fréquente.

**Le vieux centre se visite surtout à pied.** {{Hlavná ulica}}, piétonne depuis 1986, forme le plus grand centre historique protégé de Slovaquie, et la plupart des sites se trouvent à quelques minutes les uns des autres — largement de quoi pratiquer *rovno*, *vľavo* et *vpravo* en vrai.

---

## Vocabulaire complémentaire

| Slovaque | Français |
|----------|----------|
| roh | le coin |
| smer | la direction |
| ďalej | plus loin |
| naspäť | en arrière |
| križovatka | le carrefour |
| svetlá | les feux |
| chodník | le trottoir |

---

## Encore quelques phrases

! Na križovatke choďte vpravo.
> Au carrefour, allez à droite.
> Na križovatke = au carrefour
> choďte = allez
> vpravo = à droite

! Svetlá sú červené.
> Les feux sont rouges.
> Svetlá = les feux
> sú = sont
> červené = rouges

! Choďte ešte kúsok ďalej.
> Allez encore un peu plus loin.
> Choďte = allez
> ešte kúsok = encore un peu
> ďalej = plus loin

! Vráťte sa naspäť.
> Retournez en arrière.
> Vráťte sa = retournez
> naspäť = en arrière

! Chodník je úzky.
> Le trottoir est étroit.
> Chodník = le trottoir
> je = est
> úzky = étroit
