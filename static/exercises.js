...
async function renderSelectionScreen() {
    ...
    content.appendChild(el("button", {
        ...
        text: "\ud83d\uddd1\ufe0f Reset my progress",
        onclick: () => {
            if (window.confirm("Erase every score saved on this device?")) {
                resetProgress();
                renderSelectionScreen();
            }
        },
...
function startSessionFromQuery(query) {
    ...
    const n = query.get("n") ? parseInt(query.get("n"), 10) : 10;

    state.activeTypes = new Set(query.get("types") ? query.get("types").split(",") : ALL_TYPES);
    state.direction = query.get("direction") || "l1-l2";
    state.answerMode = query.get("mode") === "type" ? "type" : "choice";
    state.sessionSize = n;
